...
// Safely load dev-only Horizons platform plugins (not needed for production build)
async function loadDevPlugins() {
  ...
  try {
    ...
    console.warn('[vite] Dev plugins not available, skipping:', e.message);
    return [];
  }
}
...
console.error = function(...args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const arg = args[i];
    if (arg instanceof Error) { errorString = arg.stack || arg.name + ': ' + arg.message; break; }
  }
  if (!errorString) {
...
if (window.navigation && window.self !== window.top) {
  window.navigation.addEventListener('navigate', (event) => {
    ...
    window.parent.postMessage({ type: 'horizons-navigation-error', url }, '*');
  });
}
`;
...
const addTransformIndexHtml = {
  ...
  transformIndexHtml(html) {
    ...
    if (!isDev && process.env.TEMPLATE_BANNER_SCRIPT_URL && process.env.TEMPLATE_REDIRECT_URL) {
      tags.push({
        tag: 'script',
        attrs: { src: process.env.TEMPLATE_BANNER_SCRIPT_URL, 'template-redirect-url': process.env.TEMPLATE_REDIRECT_URL },
        injectTo: 'head',
      });
    }
    return { html, tags };
...
export default defineConfig(async () => {
  ...
  return {
    customLogger: logger,
    plugins: [...devPlugins, react(), addTransformIndexHtml],
    server: {
      cors: true,
